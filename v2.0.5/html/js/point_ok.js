...
function getPointResult(width) {
    ...
    if (width <= 767) {
        ...
        jQuery.ajax({
            url: serverUrl+"/appapi/GET_POINT_MOBILE/appRequest.do",
            type: "POST",
            dataType: 'json',
            data: {
                userId: userId,
                ...
                jQuery.ajax({
                    ...
                    if (is == 'true') {
                        ...
                        if (result.pay_method == "point") {
                            ...
                            ddate = ddate + "000";
                            $('.payInfo').html(result.vbank_name+" "+result.vbank_num+"<br>"+"예금주: 농업회사법인 신호(주) <br>유효기간: "+ new Date(Number(ddate)).getD());
                        }
                    } else if (is == 'false') {
